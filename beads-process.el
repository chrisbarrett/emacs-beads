;;; beads-process.el --- Process management and logging for bd commands -*- lexical-binding: t; -*-
...
(defvar beads-process-log-buffer-name "*bd-process*"
  "Name of the buffer used for logging bd command execution.")

(define-derived-mode beads-process-log-mode magit-section-mode "BD-Process"
...
(defun beads-process-log-buffer ()
  "Get or create the bd-process log buffer."
  (let ((buffer (get-buffer-create beads-process-log-buffer-name)))
    (with-current-buffer buffer
      (unless (eq major-mode 'beads-process-log-mode)
        (beads-process-log-mode)
...
;;;###autoload
(defun beads-process-show-log ()
  "Display the bd-process log buffer."
  (interactive)
  (pop-to-buffer (beads-process-log-buffer)))


;;; Command Queue

(defvar beads-process-commands-queue nil
  "FIFO queue of command groups received by `beads-process-call'.

Each element is a list of command plists, as received by the function.")

(defun beads-process-call (&rest commands)
...
not be called."
  (when commands
    (push commands beads-process-commands-queue)
    (when (eq 1 (length beads-process-commands-queue))
      (beads-process--pull-work-from-calls-queue))
    t))

(defun beads-process--pull-work-from-calls-queue ()
  "Take some work off the calls queue and run it."
  (when-let* ((commands (car (last beads-process-commands-queue))))
    (setq beads-process-commands-queue (butlast beads-process-commands-queue))
    (beads-process--run-sequentially commands)))

(defvar beads-proc-buffer " *bd-proc-buffer*")

(defun beads-process-command-to-process-args (command)
...
Returns a list suitable for passing to `make-process' :command parameter."
  (let* ((bd-subcommand (plist-get command :bd))
         ...
         (args (cond
                (bd-subcommand (list beads-program bd-subcommand))
...
(defun beads-process--run-sequentially (commands)
  ...
  (pcase commands
    ...
    (`(,cur-command . ,next-commands)
     (pcase-let* (((map :callback :stdin) cur-command)
                  (buf (get-buffer-create beads-proc-buffer))
                  (process-args (beads-process-command-to-process-args cur-command))
                  (process-sentinel
                   (lambda (proc status)
                     (condition-case err
                         (let ((exit-code (process-exit-status proc)))
                           (if (zerop exit-code)
                               (progn
                                 (beads-process-log--complete-command proc buf)
                                 (when callback
                                   (with-current-buffer buf
                                     (funcall callback (string-trim (buffer-string)))))
                                 (beads-process--run-sequentially next-commands))
                             (beads-process-log--fail-command proc buf status)
                             (beads-process--pull-work-from-calls-queue)))
                       (error
                        (beads-process-log--fail-command proc buf status)
       ...
       (let ((proc (make-process :name "bd"
         ...
         (beads-process-log--start-command cur-command proc)
         (when stdin
    ...
    (let ((inhibit-read-only t)
      ...
      (let* ((bd-subcommand (plist-get command :bd))
             ...
             (cmd-display (if bd-subcommand
                              (format "%s %s" beads-program bd-subcommand)
                            arbitrary-command))
             (arguments (plist-get command :arguments))
             (flags (plist-get command :flags))
             (section
        ...
        (push (cons proc section) beads-process-log--section-cache)))))

(defun beads-process-log--complete-command (proc proc-buf)
  "Log successful completion of process PROC.
PROC-BUF is the buffer containing the process output."
  (when-let* ((section (alist-get proc beads-process-log--section-cache)))
    (with-current-buffer (beads-process-log-buffer)
      (let ((inhibit-read-only t)
        ...
        (save-excursion
          ...
          (when (and output (not (string-empty-p output)))
            ...
            (unless (string-suffix-p "\n" output)
              (insert "\n"))))))
    ;; Remove from cache
    (setq beads-process-log--section-cache
          (assq-delete-all proc beads-process-log--section-cache))))

(defun beads-process-log--fail-command (proc proc-buf status)
...
(provide 'beads-process)

;;; beads-process.el ends here
